...
import combio.core
from combio.rhythm import Rhythm
import numpy as np
...
class RhythmTrial:
    ...
    def __init__(self,
                 stimuli: Union[combio.core.Stimulus, Iterable[combio.core.Stimulus]],
                 rhythm: Rhythm,
                 name: str = None):

        # Check if correct objects were passed for stimuli
        if isinstance(stimuli, combio.core.Stimulus):
    ...
    def _make_sound(self, provided_events):
        ...
        if np.max(samples) > 1:
            ...
            return combio.core.normalize_audio(samples)
        else:
            return samples

    def add_layer(self, rhythm: Rhythm, stimuli: Union[combio.core.Stimulus, Iterable[combio.core.Stimulus]]):
        ...
        if not all(isinstance(stim, combio.core.Stimulus) for stim in stimuli) or not isinstance(stimuli,
                                                                                                 combio.core.Stimulus):
    ...
    def plot_rhythm(self, filepath=None, print_staff=True, lilypond_percussion_notes=None, stem_directions=None):
        ...
        lp = combio.rhythm.helpers.get_lp_from_events(self.events,
                                                      self.n_layers,
                                                      self.time_signature,
    ...
    def plot_waveform(self, title=None):
        ...
        combio.core.plot_waveform(self.samples, self.fs, self.n_channels, title)
    ...
    def write_wav(self, out_path='.',
        ...
        combio.core.write_wav(self.samples, self.fs, out_path, self.name, metronome, self.beat_ms, metronome_amplitude)
